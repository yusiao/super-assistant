...
- Skyscanner Indicative Prices：探索未來一年或指定日期區間的最低航段價格。
...
- `SKYSCANNER_API_KEY`：全年最低與日期區間機票搜尋需要
...
- `SKYSCANNER_API_KEY`：全年最低與日期區間機票搜尋
...
Skyscanner Indicative Prices 是探索快取價，官方說可能最多落後 4 天。LINE 訊息與畫面都會標示「探索價」，購買前必須開啟連結確認即時可售價格。

機票搜尋結果會顯示「該年平均」與「去年平均」：

- 該年平均：本次搜尋中，該曆年所有可取得出發日探索價的平均。
- 去年平均：系統已累積的同航段、相同行程天數之去年每日探索樣本平均。
- Skyscanner 不提供已飛完航段的完整歷史均價；尚未累積到去年資料時，畫面會明確顯示「尚無去年資料」。
- 為節省 Cloudflare KV 額度，同航段與行程天數每天最多寫入一次年度平均樣本。

背景 workflow 可以每小時醒來，但機票來源預設 `check_interval_hours: 12`，因此每 12 小時才真正呼叫一次 Skyscanner；使用 `--force` 可略過此限制手動重查。
...
- Skyscanner Indicative API 適合找整月與多日期最低價，但不是最終可購買報價；候選日期仍需再做 live price 確認。
