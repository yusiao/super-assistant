...
const passwordIterations = Number(worker.match(/const PASSWORD_ITERATIONS = ([\d_]+)/)?.[1].replaceAll("_", ""));
assert.equal(passwordIterations, 100_000);
assert.ok(passwordIterations <= 100_000, "Cloudflare Workers rejects PBKDF2 iteration counts above 100000");
assert.match(worker, /const LEGACY_PASSWORD_ITERATIONS = 20_000/);
assert.match(worker, /storedPasswordIterations\(row\?\.password_iterations\)/);
