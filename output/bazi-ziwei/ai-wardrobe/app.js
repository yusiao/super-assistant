...
let storedItems;
try { storedItems = JSON.parse(localStorage.getItem('mori-items') || 'null'); } catch { storedItems = null; }
let items = (storedItems || demoItems).map(normalizeItem);
let saved = JSON.parse(localStorage.getItem('mori-saved') || '[]');
let filter = '全部', pendingImages = [], currentLooks = [], generationRound = 0;

function persist() {
  localStorage.setItem('mori-items', JSON.stringify(items));
  localStorage.setItem('mori-saved', JSON.stringify(saved));
}
...
function init() { renderFilters(); renderItems(); renderSaved(); updateWeather(); }
...
$('#clothesGrid').addEventListener('click', e => { const id = e.target.dataset.delete; if (!id) return; items = items.filter(x => x.id !== id); persist(); renderItems(); toast('已移出衣櫃'); });
$('#uploadTrigger').addEventListener('click', () => $('#fileInput').click());
$('#mobileUpload').addEventListener('click', () => $('#fileInput').click());
$('#fileInput').addEventListener('change', e => { pendingImages = [...e.target.files]; if (pendingImages.length) openNextUpload(); });
...
function openNextUpload() {
  ...
  const file = pendingImages.shift(), reader = new FileReader();
  reader.onload = () => { $('#uploadPreview').src = reader.result; $('#itemName').value = file.name.replace(/\.[^.]+$/, ''); estimateColor(reader.result); $('#itemDialog').showModal(); };
  reader.readAsDataURL(file);
}
$('#itemForm').addEventListener('submit', e => {
  e.preventDefault(); const name = $('#itemName').value.trim(); if (!name) return;
  items.unshift({ id: crypto.randomUUID(), name, category: $('#itemCategory').value, color: $('#itemColor').value, material: $('#itemMaterial').value, formality: +$('#itemFormality').value, style: $('#itemStyle').value, image: $('#uploadPreview').src });
  persist(); $('#itemDialog').close(); renderItems(); toast('已分析並收進衣櫃'); setTimeout(openNextUpload, 200);
});
...
$('#looksGrid').addEventListener('click', e => { if (!e.target.dataset.look) return; const look = currentLooks[+e.target.dataset.look], exists = saved.findIndex(x => x.id === look.id); if (exists >= 0) { saved.splice(exists, 1); toast('已取消收藏'); } else { saved.unshift(look); toast('已收藏這套穿搭'); } persist(); renderLooks(); renderSaved(); updateStats(); });
$('#savedGrid').addEventListener('click', e => { const key = e.target.dataset.look; if (!key?.startsWith('saved-')) return; saved.splice(+key.split('-')[1], 1); persist(); renderSaved(); updateStats(); toast('已取消收藏'); });
...
$('#resetButton').addEventListener('click', () => { items = demoItems.map(normalizeItem); saved = []; persist(); filter = '全部'; renderFilters(); renderItems(); renderSaved(); updateStats(); toast('衣櫃已恢復示範狀態'); });
